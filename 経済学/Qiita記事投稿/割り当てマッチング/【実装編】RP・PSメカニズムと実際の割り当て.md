...
本記事では割り当ての「選好 → 確率行列 → 実際の割り当て」を一気通貫で実装しました。
...
上記の内容で記事にしていますが、私自身がまだわかっていない部分もあります。例えば、$N\times M$行列を対象とした場合や二重確率行列ではない行列を対象とした場合の考え方などはまだわかっていないです。このように勉強途中ではありますがキリが良かったので現状の理解と振り返りのために記事に起こしてみました。
