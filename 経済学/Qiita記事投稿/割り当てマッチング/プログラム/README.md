...
## 一般化BvN定理（bihierarchy）

`bvn_algorithm.py` が扱えるのは「正方の二重確率行列」（＝1対1割当）に限られます。多対1・多対多や、上限制約つきの期待行列を分解するには **一般化BvN定理**（Budish, Che, Kojima and Milgrom 2013, 定理1）が必要で、それを実装したのが `generalized_bvn_algorithm.py` です。

- **制約構造 H**: 制約集合 S ⊆ N×O ごとに整数の下限・上限を与えたもの。1対1の「行和=1・列和=1」はその特殊ケース。
- **bihierarchy**: H を2つの階層（laminar family: 任意の2元が入れ子か互いに素）の和に分割できること。**行方向の族と列方向の族が、それぞれ内部で階層なら OK**。
- **定理1（十分性）**: bihierarchy なら、クォータを満たす任意の期待割当が必ず実装可能（Hoffman–Kruskal + Edmonds の完全単模性）。**BvN定理（系1）はこの1行系**。
- **定理2（必要性）**: 行制約と列制約をすべて含む「正準二部制約構造」では、bihierarchy は必要十分。二部の世界ではこれ以上の一般化はない。
- **補題1・定理12・13**: 制約集合が「奇サイクル」を成すと実装不可能。3部マッチングとルームメイト問題は必ず奇サイクルを含むため、BvN的アプローチは二部構造までが限界。

```
        1対1                          制約つき（多対1・多対多）
  bvn_algorithm.py            →   generalized_bvn_algorithm.py
  二重確率行列 → 置換行列        bihierarchy を満たす期待行列 → 純割当
```

既存の実行例4（ペア禁止制約と若手制約が同一案件上で交差する）が bihierarchy にならないことは、`find_bihierarchy()` が `None` を返し `find_odd_cycle()` が証拠の3つ組を返すことで機械的に確認できます。

理論の詳細な読解ノートは `../一般化BvN定理と多対1割り当て.md` にあります。
...
- **一般化BvN**: 各ステップで「新たに整数になる制約集合が1つ以上増える」ため深さは高々 `|H|`。ただし本実装は両枝を再帰的に展開するので、項数が最悪 `2^|H|` になりえます（論文の多項式時間アルゴリズムは乱数で片枝だけを辿り、純割当を1つサンプリングする方式）。教科書規模の例では問題ありません。
