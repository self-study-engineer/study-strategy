...
<img src="images/8-1.png" width=58%>
...
> \end{align}$$実現可能集合$U$は2人のプレイヤーが協力によって実現可能な利得ベクトル$u=(u_1,\;u_2)$の集合を表す。交渉の不一致点$d=(d_1,\;d_2)$は交渉が決裂する場合のプレイヤーの利得の組を表す。
...
- 戦略形ゲームから交渉問題を定式化する場合の困難な点は交渉の不一致点があらかじめ定まっていないことである。もちろん、ゲームの状況から交渉の不一致点が自然に定まる場合もある。また、ナッシュ均衡点やミニマックス利得のベクトルを交渉の不一致点として見做せる場合もある。しかしながら交渉の不一致点はどのような交渉ルールのもとで話し合いが行われるか、さらに、交渉が決裂した時にどのような行動が可能であるかに依存し、一般的な導出方法は確立されていない。むしろ、交渉の不一致点と妥結点が同時に定まるような、より一般的な交渉モデルを提示する必要がある。ここでは、定義8.1による交渉問題の定式化のように交渉の実現可能集合と不一致点はすでに与えられているものとする。
